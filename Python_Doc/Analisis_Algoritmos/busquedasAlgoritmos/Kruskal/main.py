...
# ── Setup ────────────────────────────────────────────────────────────────────
BASE_DIR    = os.path.dirname("/home/yo/Downloads/busquedasAlgoritmos/")
nombreMapa  = "2"
...
def camino_libre(th2_gray, v1, v2, muestras=20):
    ...
    for t in np.linspace(0, 1, muestras):
        ...
        r = np.clip(r, 0, h - 1)
        c = np.clip(c, 0, w - 1)
        if th2_gray[r, c] < 240:   # píxel oscuro → obstáculo
            return False
    return True

# ── Construcción de aristas ───────────────────────────────────────────────────
# th2 sigue siendo binario 1-canal; usarlo oara la verificacion
n      = len(vertices)
...
for v in vertices:
    cv2.circle(mst_img, (v[1], v[0]), 4, (0, 0, 255), -1)  
